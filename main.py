...
class PasswordManager(QWidget):
    ...
    def init_ui(self):
        ...
        self.service_input.setFixedHeight(40)  # Increase height
        ...
        self.save_button.setFixedHeight(40)  # Increase button height
        ...
        self.output.setMinimumHeight(150)  # Make output area larger
        ...
        if os.path.exists("passwords.json"):
            with open("passwords.json", "r") as file:
                try:
                    ...
                    QMessageBox.warning(self, "Error", "Password file is corrupted!")
                    return

                services = "\n".join(passwords.keys())
                self.output.setText("Stored Services:\n" + services if services else "No services found.")
        else:
            QMessageBox.warning(self, "Error", "No stored passwords found!")

    def retrieve_password(self):
        service, ok = QInputDialog.getText(self, "Retrieve Password", "Enter Service Name:")
        ...
        if os.path.exists("passwords.json"):
            with open("passwords.json", "r") as file:
                ...
                if service in passwords:
                    ...
                    decrypted_password = self.decrypt_password(encrypted_password.encode(), key)
                    QMessageBox.information(self, "Retrieved Password", f"Username: {passwords[service]['username']}\nPassword: {decrypted_password}")
                else:
    ...
    def delete_password(self):
        service, ok = QInputDialog.getText(self, "Delete Password", "Enter Service Name:")
        if not ok or not service:
            return
